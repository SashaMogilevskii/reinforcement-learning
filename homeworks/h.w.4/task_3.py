...
# Experiment 1
# def calculate_epsilon(episode, episode_n):
#
#     epsilon = 1 - episode / episode_n
#     return max(epsilon, 0.25)

# Experiment 2
# def calculate_epsilon(episode, episode_n):
#     linear_decay = 1 - episode / episode_n
#     cosine_decay = 0.5 * (1 + np.cos(episode * np.pi / episode_n))
#     epsilon = max(linear_decay * cosine_decay, 0.05)
#     return epsilon
...
def calculate_epsilon(episode, episode_n):
    if episode <= 200:
        epsilon = 1.0 - episode / 200.0
    else:
        linear_decay = 1 - (episode + 100) / (episode_n)
        cosine_decay = 0.5 * (1 + np.cos(episode * np.pi / episode_n))
        epsilon = max(linear_decay * cosine_decay, 0.003)
    return epsilon
def MonteCarlo(env, episode_n, trajectory_len=500, gamma=0.99):
    ...
    for episode in range(episode_n):
        epsilon = calculate_epsilon(episode=episode, episode_n=episode_n)
